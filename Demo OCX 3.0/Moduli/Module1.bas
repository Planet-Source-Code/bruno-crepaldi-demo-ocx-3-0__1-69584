...
Public msk As Long, MSKO1 As Long, MSKI As BITMAPINFO, MSKBITS() As Byte
Public nSRC As Long, nSRCO1 As Long, nSRCI As BITMAPINFO, SRCBITS() As Byte
Public DST As Long, DSTO1 As Long, DSTI As BITMAPINFO, DSTBITS() As Byte
Public BB As Long, BBO As Long

Public LX As Long, LY As Long

Public Sub Setup(myform As Form, S1 As Long, S2 As Long)
'set the width and height
W = myform.src1.ScaleWidth
H = myform.src1.ScaleHeight
...
BBO = CreateCompatibleBitmap(GetDC(0), myform.ScaleWidth, myform.ScaleHeight)
...
BitBlt BB, 0, 0, myform.ScaleWidth, myform.ScaleHeight, myform.hdc, 0, 0, vbSrcCopy
...
Public Sub BLTIT(myform As Form, ByVal X As Long, ByVal Y As Long)
...
If X >= myform.ScaleWidth - (W + 1) Then
    X = myform.ScaleWidth - (W + 1)
End If
If Y >= myform.ScaleHeight - (H + 1) Then
    Y = myform.ScaleHeight - (H + 1)
End If
If Y <= 0 Then
    Y = 0
End If
...
For i = 1 To UBound(SRCBITS) Step 3
    ...
    'values are the saMyform. with a full color mask,
...
BitBlt myform.hdc, LX, LY, W, H, BB, LX, LY, vbSrcCopy

'blt the alpha values to the screen
SetDIBitsToDevice myform.hdc, X, Y, W, H, 0, 0, 0, H, tmpBits(1), nSRCI, DIB_RGB_COLORS

'set the Last X and Last Y values so we know where
'tp clear the screen next tiMyform.
LX = X
LY = Y
End Sub
